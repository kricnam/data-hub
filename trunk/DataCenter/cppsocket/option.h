...
namespace CPPSocket
{
  ...
  protected:
    virtual char* pointer() throw()             = 0; ///< write access to the option
    virtual const char* pointer() const throw() = 0; ///< read access to the option
...
//  protected:
    virtual char* pointer() throw()             { return reinterpret_cast<char*>(&value); }
    virtual const char* pointer() const throw() { return reinterpret_cast<const char*>(&value); }
...
#endif
#ifdef SO_SNDTIMEO
    typedef OptionTemplate<SOL_SOCKET, SO_SNDTIMEO,  struct timeval>    SndTimeO  ;
#endif
#ifdef SO_BSDCOMPAT
